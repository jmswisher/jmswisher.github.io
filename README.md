...
- Shared CSS
- JSON data files for site config, positions, documents, samples, and selections of doc samples per position. (Document samples are "lenses" on documents: the same document can be viewed and described in multiple ways.)
...
2. Update `documents.json` and `samples.json` with metadata about the samples.
